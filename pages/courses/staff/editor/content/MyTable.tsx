...
const MyTable = () => {
  const [data, setData] = useState([
    { uuid: uuidv4(), name: <Editable defaultValue='Page 1'><EditablePreview /><EditableInput /></Editable>},
  ]);

  const addRow = () => {
    setData([...data, { uuid: uuidv4(), name: <Editable defaultValue='New Page'><EditablePreview /><EditableInput /></Editable>}]);
  };
          ...
          <Tbody>
            {data.map((row) => (
              <Tr key={row.uuid}>
                ...
                <Td>
                <Text align='right' onClick={() => deleteRow(row.uuid)}>x</Text>
                </Td>
              </Tr>))}
              <Tr>
                <Td>
